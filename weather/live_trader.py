...
class LiveTrader:
    ...
    def execute_signal(self, signal: Signal) -> dict | None:
        """
        Place a live order for signal. Returns order info dict or None if skipped.
        Raises RuntimeError on hard blocks (gates not passed, kill switch, bad creds).

        Writes real fills to live_trades.csv ONLY. The paper/model track is owned
        by the scan loop (run_scan / mode_intraday), which logs every actionable
        signal independent of whether it fills — so the go-live gate and the data
        we optimize against never hinge on execution (a geoblock once silently
        froze the whole mainline paper track for days, Aug 2026).
        """
